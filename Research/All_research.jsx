...
<div>
    <AA/>
      <div className="container-fluid mt-4">
        <h3 className="text-center mb-3">All Student Research Papers and Contributions</h3>
        <div className="table-responsive">
          <table className="table table-sm table-bordered table-striped w-100">
            <thead className="table-dark">
              <tr>
                <th>S.no</th>
                <th>Author names (all)</th>
                <th>Author names (VFSTR)</th>
            ...
            <tbody>
              {data.map((item, index) => (
                <ResearchRow key={index} item={item} />
              ))}
            </tbody>
          </table>
        </div>
...
const ResearchRow = ({ item }) => {
  ...
  const abstractPreview = item.Abstract.split(" ").slice(0,20).join(" ");

  return (
    <tr>
      <td>{item.index}</td>
      <td>
        {item["Author names (all)"].split("\n").map((name, idx) => (
          <div key={idx}>{name}</div>
        ))}
      </td>
      <td>{item["Author names (VFSTR)"]}</td>
      <td>
        {item["Student Regd.no"].split("\n").map((reg, idx) => (
          <div key={idx}>{reg}</div>
        ))}
      </td>
      <td>{item.Title}</td>
      ...
      <td>
        {isExpanded ? item.Abstract : abstractPreview + "..."}
        <button
